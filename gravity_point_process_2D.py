...
class Gravity_point_process_2D():
    ...
    def plot_point_process(self, arg):
        ...
        if arg not in ["poisson", "push", "gravity_equilibrium", "all"] :
            raise ValueError("arg should be one of the following str: 'poisson', 'push', 'gravity_equilibrium'.")
        if arg == "poisson":
            plt.figure(figsure(5,5))
            plt.scatter(np.real(self.pois_data_c), np.imag(self.pois_data_c), s=1, c='k')
            plt.title("Poison point process")
        if arg == "push":
            plt.figure(figsure(5,5))
            plt.scatter(np.real(self.push_data_c), np.imag(self.push_data_c), s=1, c='k')
            plt.title("Push point process")
        if arg == "gravity_equilibrium":
            plt.figure(figsure(5,5))
            plt.scatter(np.real(self.equi_data_c), np.imag(self.equi_data_c), s=1, c='k')
            plt.title("Gravity equilibrium point process")
        if arg == "all":
            fig , ax = plt.subplots(1, 3, figsize=(24, 7))
            ax[0].scatter(np.real(self.pois_data_c), np.imag(self.pois_data_c), s=1, c='k')
            ax[0].title.set_text("Poisson point process")
            ax[1].scatter(np.real(self.push_data_c), np.imag(self.push_data_c), s=1, c='k')
            ax[1].title.set_text('Push point process')
            ax[2].scatter(np.real(self.equi_data_c), np.imag(self.equi_data_c), s=1, c='k')
